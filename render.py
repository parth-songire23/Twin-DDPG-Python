...
class Arrow3D(FancyArrowPatch):
    def __init__(self, xs, ys, zs, ax, *args, **kwargs):
        ...
        self.ax = ax  # Store the 3D axis reference

    def draw(self, renderer):
        # Use the ax's projection matrix instead of the deprecated renderer.axes.M
        xs3d, ys3d, zs3d = self._verts3d
        proj_matrix = self.ax.get_proj()
        xs, ys, zs = proj3d.proj_transform(xs3d, ys3d, zs3d, proj_matrix)
...
class Render(object):
    ...
    def plot_one_channel(self, ax, channel, color, text="channel"):
        """
        Function used in plot_channels to show only one channel.
        """        
        ...
        # Pass ax as an argument to Arrow3D
